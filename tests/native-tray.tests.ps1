...
Assert-Contains $source 'STARTF_USESTDHANDLES | STARTF_USESHOWWINDOW' 'Launcher should explicitly hide the server console window.'
Assert-Contains $source 'startup.wShowWindow = SW_HIDE;' 'Launcher should ask for the server console to start hidden.'
Assert-Contains $source 'TRUE, CREATE_NO_WINDOW, NULL, NULL, &startup, &process' 'Launcher should start the server with a console that has no window, so descendants inherit a console that cannot flash.'
Assert-Contains $source 'static bool IsServerProcess(DWORD pid)' 'Console hiding should be scoped to the launcher process tree.'
Assert-Contains $source 'static void HideForeignConsoleWindow(HWND hwnd)' 'Launcher should hide a console window its own process tree created.'
Assert-Contains $source 'static bool TreeOwnsImagePath(const wchar_t *title)' 'Launcher should recognise a console a delegated terminal hosts for its own tree.'
Assert-Contains $source 'CASCADIA_HOSTING_WINDOW_CLASS' 'Launcher should handle a console window hosted by Windows Terminal.'
Assert-Contains $source 'QueryFullProcessImageNameW(process, 0, image, &size)' 'The delegated-terminal test should compare the window title against a real image path.'
Assert-Contains $source 'SetWinEventHook(EVENT_OBJECT_CREATE, EVENT_OBJECT_NAMECHANGE' 'Launcher should watch console windows being created, shown, and renamed.'
Assert-Contains $source 'event != EVENT_OBJECT_NAMECHANGE' 'Launcher should hide a delegated terminal window as soon as it is renamed to the console client.'
Assert-Contains $source 'OBJID_WINDOW' 'The console-window hook should ignore non-window objects.'
Assert-Contains $source 'UnhookWinEvent(g_console_hook)' 'Launcher should release the console-window hook when it exits.'
